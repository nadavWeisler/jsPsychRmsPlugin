...
jsPsych.pluginAPI = (function () {
    ...
    let root_keydown_listener = function (e) {
    ...
    module.preloadImages = function (images, callback_complete, callback_load) {
        ...
        let n_loaded = 0;
        let loadfn = (typeof callback_load === 'undefined') ? function () {
        } : callback_load;
        let finishfn = (typeof callback_complete === 'undefined') ? function () {
        } : callback_complete;
    ...
    module.autoPreload = function (timeline, callback, images, audio, progress_bar) {
        ...
        for (let i = 0; i < preloads.length; i++) {
            let type = preloads[i].plugin;
            let param = preloads[i].parameter;
            let media = preloads[i].media_type;
            let func = preloads[i].conditional_function;
            let trials = timeline.trialsOfType(type);
